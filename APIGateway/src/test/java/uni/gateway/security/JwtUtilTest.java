...
class JwtUtilTest {
	...
	void null_token_returns_false_without_exception() {
		assertThatCode(() -> jwtUtil.isValid(null)).doesNotThrowAnyException();
	...
	void expired_token_is_not_valid() {
		String expired = io.jsonwebtoken.Jwts.builder().subject("user-123").issuedAt(new java.util.Date(0))
				.expiration(new java.util.Date(1))
				.signWith(io.jsonwebtoken.security.Keys.hmacShaKeyFor(io.jsonwebtoken.io.Decoders.BASE64
						.decode("dGVzdFNlY3JlddGVzdFNlY3JldEtleUZvckpXVFRlc3RpbmcxMjMEtleUZvckpXVFRlc3RpbmcxMjM=")))
				.compact();
	...
	void extract_user_id_throws_on_invalid_token() {
		assertThatThrownBy(() -> jwtUtil.extractUserId("bad.token.here")).isInstanceOf(Exception.class);
	}
}
