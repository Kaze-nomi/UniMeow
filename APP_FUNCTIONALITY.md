...
- Корневые комментарии поста возвращаются в рекомендационном порядке:
...
- Ответы внутри ветки родительского комментария возвращаются в хронологическом порядке (`createdAt ASC`, затем `id ASC`).
- Score корневых комментариев считается в `PostService` при чтении из актуальных значений `createdAt` и `likesCount`; отдельная Redis-проекция для комментариев не используется.
- При лайке/анлайке корневого комментария меняется `likesCount`, поэтому следующий `getComments` возвращает порядок корневых комментариев с пересчитанным score. Лайки ответов не меняют хронологический порядок ответов.
...
- Комментарии: корневые комментарии показываются в рекомендационном порядке, ответы внутри ветки показываются хронологически; создание, редактирование, удаление, лайк/анлайк; после лайка/анлайка страница поста синхронизирует состояние лайка и порядок комментариев с backend.
