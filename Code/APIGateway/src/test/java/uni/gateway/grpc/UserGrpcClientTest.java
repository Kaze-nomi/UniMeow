...
class UserGrpcClientTest {
    ...
    void create_or_get_user_returns_user_response() {
        ...
        StepVerifier.create(client.createOrGetUser(
                        CreateOrGetUserRequest.newBuilder()
                                .setEmailGoogle("ivan@gmail.com")
                                .setName("Ivan")
                                .setSurname("Ivanov")
                                .setAvatarUrl("https://img")
                                .build()))
                .assertNext(response ->
    ...
    void create_or_get_user_propagates_grpc_error() {
        when(stub.createOrGetUser(any()))
                .thenThrow(Status.INTERNAL.withDescription("DB error").asRuntimeException());

        StepVerifier.create(client.createOrGetUser(
                        CreateOrGetUserRequest.newBuilder().build()))
    ...
    void get_user_by_id_sends_correct_request() {
        ...
        StepVerifier.create(client.getUserById(USER_ID))
                .expectNextCount(1)
                .verifyComplete();
    ...
    void get_user_by_username_sends_correct_request() {
        ...
        ArgumentCaptor<GetUserByUsernameRequest> captor =
                ArgumentCaptor.forClass(GetUserByUsernameRequest.class);

        StepVerifier.create(client.getUserByUsername("iivanov"))
                .expectNextCount(1)
                .verifyComplete();
    ...
    void update_user_returns_updated_response() {
        ...
        StepVerifier.create(client.updateUser(request))
                .assertNext(r -> assertThat(r.getName()).isEqualTo("Petr"))
                .verifyComplete();
    }

    @Test
    void update_user_propagates_already_exists_when_username_taken() {
        when(stub.updateUser(any()))
                .thenThrow(Status.ALREADY_EXISTS.withDescription("taken").asRuntimeException());

        StepVerifier.create(client.updateUser(
    ...
    void create_session_sends_correct_request_and_completes() {
        when(stub.createSession(any()))
                .thenReturn(CreateSessionResponse.newBuilder().setSuccess(true).build());

        ArgumentCaptor<CreateSessionRequest> captor =
                ArgumentCaptor.forClass(CreateSessionRequest.class);

        StepVerifier.create(client.createSession(USER_ID, "refresh-token-xyz"))
                .verifyComplete();
    ...
    void create_session_propagates_grpc_error() {
        ...
        StepVerifier.create(client.createSession(USER_ID, "token"))
    ...
    void refresh_session_sends_correct_request() {
        RefreshSessionResponse grpcResponse = RefreshSessionResponse.newBuilder()
                .setUserId(USER_ID)
                .setNewRefreshToken("new-rt")
                .build();
        when(stub.refreshSession(any())).thenReturn(grpcResponse);

        ArgumentCaptor<RefreshSessionRequest> captor =
                ArgumentCaptor.forClass(RefreshSessionRequest.class);

        StepVerifier.create(client.refreshSession("old-rt"))
                .assertNext(r -> {
    ...
    void revoke_refresh_token_returns_true_on_success() {
        ...
        StepVerifier.create(client.revokeRefreshToken("rt"))
    ...
    void revoke_refresh_token_sends_correct_request() {
        ...
        ArgumentCaptor<RevokeRefreshTokenRequest> captor =
                ArgumentCaptor.forClass(RevokeRefreshTokenRequest.class);

        StepVerifier.create(client.revokeRefreshToken("my-token"))
                .expectNextCount(1)
                .verifyComplete();
