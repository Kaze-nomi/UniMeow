...
import uni.gateway.dto.SubscribeResult;
import uni.gateway.dto.UpdateProfileInput;
import uni.gateway.dto.UserDto;
import uni.gateway.dto.VerificationResult;
import uni.gateway.dto.VerifyResult;
...
public class UserController {
    ...
    @MutationMapping
    public Mono<VerificationResult> sendVerificationCode(
            @Argument String universityEmail,
            @ContextValue(required = false) String userId) {
        if (userId == null) {
            return Mono.error(new CredentialException("Authentication required"));
        }

        return userGrpcClient.sendVerificationCode(userId, universityEmail)
                .map(VerificationResult::new);
    }

    @MutationMapping
    public Mono<VerifyResult> verifyEmailCode(
        ...
        return userGrpcClient.verifyEmailCode(userId, code)
                .map(resp -> new VerifyResult(
                        resp.getSuccess(),
                        resp.getError().isBlank() ? null : resp.getError()));
    }

    @MutationMapping
    public Mono<SubscribeResult> subscribe(
            @Argument String targetUserId,
            @ContextValue(required = false) String userId) {
        if (userId == null) {
            return Mono.error(new CredentialException("Authentication required"));
        }

        return userGrpcClient.subscribe(userId, targetUserId)
                .map(SubscribeResult::new);
    }

    @MutationMapping
    public Mono<SubscribeResult> unsubscribe(
            @Argument String targetUserId,
            @ContextValue(required = false) String userId) {
        if (userId == null) {
            return Mono.error(new CredentialException("Authentication required"));
        ...
        return userGrpcClient.unsubscribe(userId, targetUserId)
                .map(SubscribeResult::new);
    }
