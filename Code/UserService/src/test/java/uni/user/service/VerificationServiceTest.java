...
class VerificationServiceTest {
    ...
    private UniversityDomain studentDomain() {
        return new UniversityDomain(1L, "student.mgu.ru",
                UniversityDomain.DomainRole.STUDENT, "МГУ");
    }

    private UniversityDomain employeeDomain() {
        return new UniversityDomain(2L, "mgu.ru",
                UniversityDomain.DomainRole.EMPLOYEE, "МГУ");
    }

    @Test
    void send_code_saves_verification_record() {
        when(domainRepository.findByDomain("student.mgu.ru"))
    ...
    void send_code_throws_when_domain_is_unknown() {
        when(domainRepository.findByDomain("gmail.com"))
                .thenReturn(Optional.empty());

        assertThatThrownBy(() ->
                verificationService.sendCode(USER_ID, "ivan@gmail.com"))
                .isInstanceOf(UnknownDomainException.class)
    ...
    void send_code_throws_when_email_already_used_by_another_user() {
        ...
        User otherUser = User.builder()
                .id(otherUserId)
                .emailGoogle("other@gmail.com")
                .emailUniversity("ivan@student.mgu.ru")
                .name("Other")
                .createdAt(LocalDateTime.now())
                .build();
    ...
    void verify_returns_success_and_sets_employee_verified() {
        ...
        when(verifyRepository.findById(USER_ID)).thenReturn(Optional.of(record));
        when(domainRepository.findByDomain("mgu.ru"))
                .thenReturn(Optional.of(employeeDomain()));
        when(userRepository.findById(USER_ID)).thenReturn(Optional.of(user));
        when(userRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
