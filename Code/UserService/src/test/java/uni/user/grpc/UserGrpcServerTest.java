...
class UserGrpcServerTest {
    ...
    void get_user_by_id_returns_user_response_on_success() {
        ...
        server.getUserById(
                GetUserByIdRequest.newBuilder().setId(USER_ID.toString()).build(), obs);
    ...
    void get_user_by_id_returns_invalid_argument_on_bad_uuid() {
        StreamObserver<UserResponse> obs = mock();
        server.getUserById(
                GetUserByIdRequest.newBuilder().setId("not-a-uuid").build(), obs);
    ...
    void update_user_returns_already_exists_when_username_taken() {
        when(userService.update(any(), anyBoolean(), anyString(),
    ...
    void create_session_returns_success_true() {
        doNothing().when(sessionService)
                .createSession(any(UUID.class), anyString(), anyLong());
        ...
        ArgumentCaptor<CreateSessionResponse> captor =
                ArgumentCaptor.forClass(CreateSessionResponse.class);
        verify(obs).onNext(captor.capture());
        verify(obs).onCompleted();
    ...
    void refresh_session_returns_new_token_and_user_id() {
        when(sessionService.rotateRefreshToken("old-rt"))
                .thenReturn(new String[]{USER_ID.toString(), "new-rt"});

        StreamObserver<RefreshSessionResponse> obs = mock();
        server.refreshSession(
                RefreshSessionRequest.newBuilder()
                        .setOldRefreshToken("old-rt")
                        .build(), obs);

        ArgumentCaptor<RefreshSessionResponse> captor =
                ArgumentCaptor.forClass(RefreshSessionResponse.class);
        verify(obs).onNext(captor.capture());
        verify(obs).onCompleted();
    ...
    void refresh_session_returns_unauthenticated_when_expired() {
        when(sessionService.rotateRefreshToken("expired-rt"))
                .thenThrow(new SessionExpiredException("expired"));

        StreamObserver<RefreshSessionResponse> obs = mock();
        server.refreshSession(
                RefreshSessionRequest.newBuilder()
                        .setOldRefreshToken("expired-rt")
                        .build(), obs);
    ...
    void revoke_refresh_token_returns_success_true() {
        ...
        server.revokeRefreshToken(
                RevokeRefreshTokenRequest.newBuilder()
                        .setRefreshToken("rt")
                        .build(), obs);

        ArgumentCaptor<RevokeRefreshTokenResponse> captor =
                ArgumentCaptor.forClass(RevokeRefreshTokenResponse.class);
        verify(obs).onNext(captor.capture());
        verify(obs).onCompleted();
        assertThat(captor.getValue().getSuccess()).isTrue();
    }
