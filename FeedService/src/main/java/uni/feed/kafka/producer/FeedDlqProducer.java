package uni.feed.kafka.producer;
...
@Slf4j
public class FeedDlqProducer {

	private final KafkaTemplate<String, String> kafkaTemplate;
	private final ObjectMapper objectMapper;

	@Value("${app.kafka.topics.feed-events-dlq:feed-events.dlq}")
	private String feedEventsDlqTopic;
	...
	public boolean publishConsumerFailure(String sourceTopic, String key, String rawPayload, int partition, long offset,
		...
		try {
			...
			kafkaTemplate.send(feedEventsDlqTopic, key == null ? sourceTopic : key, message).join();
			return true;
		} catch (Exception e) {
			log.error("Failed to publish event to DLQ topic {}", feedEventsDlqTopic, e);
			return false;
		}
	}

	private JsonNode tryReadJson(String rawPayload) {
		try {
