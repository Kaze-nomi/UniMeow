...
class SessionServiceTest {
	...
	void rotate_returns_new_user_id_and_new_token() {
		RefreshSession session = RefreshSession.builder().userId(USER_ID).refreshToken("old-token")
	...
	void rotate_throws_when_token_not_found() {
		when(sessionRepository.findByRefreshToken("unknown-token")).thenReturn(Optional.empty());

		assertThatThrownBy(() -> sessionService.rotateRefreshToken("unknown-token"))
				.isInstanceOf(SessionExpiredException.class).hasMessageContaining("not found");
	}

	@Test
	void rotate_throws_when_token_is_expired() {
		RefreshSession expiredSession = RefreshSession.builder().userId(USER_ID).refreshToken("expired-token")
				.expiresAt(LocalDateTime.now().minusDays(1)).build();

		when(sessionRepository.findByRefreshToken("expired-token")).thenReturn(Optional.of(expiredSession));

		assertThatThrownBy(() -> sessionService.rotateRefreshToken("expired-token"))
				.isInstanceOf(SessionExpiredException.class).hasMessageContaining("expired");
	}
	...
	void revoke_token_does_not_throw_when_token_not_found() {
		...
		assertThatCode(() -> sessionService.revokeToken("ghost-token")).doesNotThrowAnyException();
	}
}
