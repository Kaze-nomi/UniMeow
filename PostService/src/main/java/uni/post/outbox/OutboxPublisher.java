...
public class OutboxPublisher {
	...
	@Value("${app.kafka.topics.post-events-dlq:post-events.dlq}")
	private String postEventsDlqTopic;
	...
	private boolean publishToDlq(OutboxEvent event, Exception sourceError) {
		try {
			...
			kafkaTemplate.send(postEventsDlqTopic, event.getEventKey(), dlqPayload).join();
			return true;
		} catch (Exception e) {
